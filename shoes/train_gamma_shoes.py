...
from utils import ShoesDataSet, ShoesImgDataSet, OutlierLossL2, OutlierLossLogistic, Identity
...
parser.add_argument('-e', '--max_epoch', type=int, default=5)
...
distinct_pair_num, all_pair_num, pairs_per_attr = process.cnt_pair_num(
    train_file)
...
if args.model == 'resnet':
    ...
    train_batch_size = 32
    test_batch_size = 32
    # dicts = torch.load('./naive_resnet_checkpt_120.pkl')
...
train_img_data = ShoesImgDataSet(txt=train_file, file_dict=FILEDICT,
                                 transform=train_transform)
...
# ================= loss function =====================
gammas = torch.zeros((args.attrib_num, pairs_per_attr[0], 2))
gammas_n = torch.zeros((args.attrib_num, pairs_per_attr[0], 2))
gammas_old = torch.zeros((args.attrib_num, pairs_per_attr[0], 2))
strengths = torch.zeros((args.attrib_num, pairs_per_attr[0], 2))
...
optimizer_a = optim.Adam(params_a, lr=lr, weight_decay=args.weight_decay)
...
with open(OUTLIERPATH, 'w') as f:
    for i in range(args.attrib_num):
        for j in range(pairs_per_attr[0]):
