...
from utils import LFWDataSet, LFWImgDataSet, LinearLoss, logitloss, huberloss, robust_logit_loss, Identity
...
parser.add_argument('--modname', type=str, default='lfw', help='dataset name')
...
def test(epoch, set_flag='test'):
    ...
    with torch.no_grad():
        for step, (_, _, img1, img2, label, attr_id, pair_id, strength) in enumerate(dataloader):
            ...
            pred = (test_score1 - test_score2).view_as(label)
            if set_flag == 'test':
                for k in range(bz):
